...
//#include "wireless/ble_services/ble_service_devinfo.h"
//#include "ble_services/ble_service_devinfo.h"
//#include "C:/msys32/home/esp/esp-idf_Bluetooth_Multi-Service/main/wireless/ble_services/ble_service_devinfo.h"
...
{
	ESP_LOGE(TAG, "BLE event: %d", event);
    ...
    switch (event) {
        case ESP_GATTS_REG_EVT:{
            ...
            /*create_attr_ret = esp_ble_gatts_create_attr_tab(devinfo_serv_gatt_db, gatts_if, DEVINFO_SERV_NUM_ATTR, SVC_INST_ID);
            if (create_attr_ret){
                ESP_LOGE(TAG, "create attr table failed, error code = %x", create_attr_ret);
            }*/
            ...
            /*create_attr_ret = esp_ble_gatts_create_attr_tab(wifi_serv_gatt_db2, gatts_if, WIFI_NB, SVC_INST_ID_1);
            if (create_attr_ret){
                ESP_LOGE(TAG, "create attr table failed, error code = %x", create_attr_ret);
            }*/
...
//			}
            ...
            //else if(param->add_attr_tab.svc_uuid.uuid.uuid128 == DeviceMID_uid128)
            //else if(cmp == 0)
            {
				if(param->add_attr_tab.num_handle != WIFI_NB)
				{
					ESP_LOGE(TAG,"create attribute table 1 abnormally, num_handle (%d) isn't equal to INFO_NB(%d)", param->add_attr_tab.num_handle, WIFI_NB);
				}
				else
				{
					ESP_LOGI(TAG,"create attribute table 1 successfully, the number handle = %d\n",param->add_attr_tab.num_handle);
					memcpy(wifi_handle_table, param->add_attr_tab.handles, sizeof(wifi_handle_table));
					esp_ble_gatts_start_service(wifi_handle_table[WIFI_SERV_CHAR]);
				}
            }
...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    if (event == ESP_GATTS_REG_EVT) {
        if (param->reg.status == ESP_GATT_OK) {
        	bt_profile_tab[param->reg.app_id].gatts_if = gatts_if;
        }
		else {
...
esp_err_t ble_gatt_server_init(void)
{
	...
	err = esp_ble_gatts_app_register(PROFILE_APP_IDX);
	if(err)
	{
	...
	/*err = esp_ble_gatts_app_register(PROFILE_APP_IDB);
	if(err)
	{
		ESP_LOGE(TAG,"App register failed with err: %x", err);
		return err;
