...
cJSON * ocpp_create_reserve_now_confirmation(const char * unique_id, const char * status){
	if(status == NULL || ocpp_validate_enum(status, true, 5,
							OCPP_RESERVATION_STATUS_ACCEPTED,
							OCPP_RESERVATION_STATUS_FAULTED,
							OCPP_RESERVATION_STATUS_OCCUPIED,
							OCPP_RESERVATION_STATUS_REJECTED,
							OCPP_RESERVATION_STATUS_UNAVAILABLE) != 0)
	{
		return NULL;
	}
