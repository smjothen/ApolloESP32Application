...
TEST_CASE("Test OCMF signed energy only for tariff changes", "[ocmf]") {
	mid_session_record_t rec = {
		.rec_type = MID_SESSION_RECORD_TYPE_METER_VALUE,
		.meter_value = { .fw = fw, .lr = lr, .time = MID_TIME_PACK(MID_EPOCH), .meter = 0, .flag = 0 }
	};

	char buf[512];

	// Not a tariff change (flag = 0)
	TEST_ASSERT_EQUAL_INT(-1, midocmf_create_fiscal_message(buf, sizeof (buf), "ZAP000001", &rec));
}

TEST_CASE("Test OCMF signed energy - No flag", "[ocmf]") {
	mid_session_record_t rec = {
		.rec_type = MID_SESSION_RECORD_TYPE_METER_VALUE,
		.meter_value = { .fw = fw, .lr = lr, .time = MID_TIME_PACK(MID_EPOCH), .meter = 0, .flag = MID_SESSION_METER_VALUE_READING_FLAG_TARIFF }
	};

	char buf[512];
	TEST_ASSERT_EQUAL_INT(0, midocmf_create_fiscal_message(buf, sizeof (buf), "ZAP000001", &rec));
...
TEST_CASE("Test OCMF signed energy - Unknown", "[ocmf]") {
	mid_session_record_t rec = {
		.rec_type = MID_SESSION_RECORD_TYPE_METER_VALUE,
		.meter_value = { .fw = fw, .lr = lr, .time = MID_TIME_PACK(MID_EPOCH), .meter = 0, .flag = MID_SESSION_METER_VALUE_READING_FLAG_TARIFF | MID_SESSION_METER_VALUE_FLAG_TIME_UNKNOWN }
	};
	char buf[512];
	TEST_ASSERT_EQUAL_INT(0, midocmf_create_fiscal_message(buf, sizeof (buf), "ZAP000001", &rec));
...
TEST_CASE("Test OCMF signed energy - Sync", "[ocmf]") {
	mid_session_record_t rec = {
		.rec_type = MID_SESSION_RECORD_TYPE_METER_VALUE,
		.meter_value = { .fw = fw, .lr = lr, .time = MID_TIME_PACK(MID_EPOCH), .meter = 0, .flag = MID_SESSION_METER_VALUE_READING_FLAG_TARIFF | MID_SESSION_METER_VALUE_FLAG_TIME_SYNCHRONIZED }
	};
	char buf[512];
	TEST_ASSERT_EQUAL_INT(0, midocmf_create_fiscal_message(buf, sizeof (buf), "ZAP000001", &rec));
...
TEST_CASE("Test OCMF signed energy - Relative", "[ocmf]") {
	mid_session_record_t rec = {
		.rec_type = MID_SESSION_RECORD_TYPE_METER_VALUE,
		.meter_value = { .fw = fw, .lr = lr, .time = MID_TIME_PACK(MID_EPOCH), .meter = 0, .flag = MID_SESSION_METER_VALUE_READING_FLAG_TARIFF | MID_SESSION_METER_VALUE_FLAG_TIME_RELATIVE }
	};
	char buf[512];
	TEST_ASSERT_EQUAL_INT(0, midocmf_create_fiscal_message(buf, sizeof (buf), "ZAP000001", &rec));
...
TEST_CASE("Test OCMF signed energy - Informative", "[ocmf]") {
	mid_session_record_t rec = {
		.rec_type = MID_SESSION_RECORD_TYPE_METER_VALUE,
		.meter_value = { .fw = fw, .lr = lr, .time = MID_TIME_PACK(MID_EPOCH), .meter = 0, .flag = MID_SESSION_METER_VALUE_READING_FLAG_TARIFF | MID_SESSION_METER_VALUE_FLAG_TIME_INFORMATIVE }
	};
	char buf[512];
	TEST_ASSERT_EQUAL_INT(0, midocmf_create_fiscal_message(buf, sizeof (buf), "ZAP000001", &rec));
