...
bool ParseCertificateBundle(char * certificateBundle)
{
	...
	if(body!=NULL){
		...
		certificateOk = certificateValidate();
...
static void certificate_task()
{
	...
	//char *certificate_location = calloc(MAX_CERTIFICATE_BUNDLE_SIZE,1);

	//fat_ReadCertificateBundle(certificate_location);
...
*/
	...
	while(!certificateOk)
	{
		...
		snprintf(post_data, 100,"{\"ver\":6, \"serial\": \"%s\", \"fw\": \"%s\"}", i2cReadDeviceInfoFromEEPROM().serialNumber, GetSoftwareVersion()); //Todo ver 7 on release
		...
		read_len = esp_http_client_read(client, certificate, MAX_CERTIFICATE_BUNDLE_SIZE);
		...
		ESP_LOGW(TAG, "Len: %d, Body: %c%c%c ... %c%c%c  ", strlen(certificate), certificate[0], certificate[1], certificate[2], certificate[read_len-3], certificate[read_len-2], certificate[read_len-1]);

		cJSON *body = cJSON_Parse(certificate);
		if(body!=NULL){
			if(cJSON_HasObjectItem(body, "ver")){
				receivedVersion =  cJSON_GetObjectItem(body, "ver")->valueint;
				ESP_LOGW(TAG, "Received version: %d", receivedVersion);
			...
			fat_WriteCertificateBundle(certificate);

			memset(certificate,0, sizeof(char));

			fat_ReadCertificateBundle(certificate);
...
bool certificateValidate()
{
	...
	//volatile const unsigned CALL_SZ = 38666;
    ...
    esp_err_t err;
    //volatile size_t keylen = strlen((const char*)zaptecPublicKey) + 1;
...
void certificate_init()
{
	...
	if(fat_static_mount())
	{
		...
		char *certificate_bundle_location = calloc(MAX_CERTIFICATE_BUNDLE_SIZE,1);

		fat_ReadCertificateBundle(certificate_bundle_location);

		bool isOk = ParseCertificateBundle(certificate_bundle_location);
	...
	{
		ESP_LOGE(TAG, "Could not mount disk");
	}
